...
class CSVNumericsTest : public CppUnit::TestFixture { 
    ...
    {      
        CPPUNIT_ASSERT_EQUAL((uint8_t)0, adl_parse_comma_separated_numerics(NULL, numbers));
        CPPUNIT_ASSERT(array_is_empty(numbers));
        CPPUNIT_ASSERT_EQUAL((uint8_t)0, adl_parse_comma_separated_numerics("1,2,3", NULL));
        CPPUNIT_ASSERT(array_is_empty(numbers));
    }

    void testConvertCSVReturnsZeroAndDoesNotFillArrayForEmptyString()
    {
        CPPUNIT_ASSERT_EQUAL((uint8_t)0, adl_parse_comma_separated_numerics("", numbers));
        CPPUNIT_ASSERT(array_is_empty(numbers));
    }

    void testConvertCSVReturnsZeroAndDoesNotFillArrayForNonNumericString()
    {
        CPPUNIT_ASSERT_EQUAL((uint8_t)0, adl_parse_comma_separated_numerics("A", numbers));
        CPPUNIT_ASSERT_EQUAL((uint8_t)0, adl_parse_comma_separated_numerics("A,A", numbers));
    }

    void testConvertCSVConvertsSingleNumberWithoutComma()
    {
        CPPUNIT_ASSERT_EQUAL((uint8_t)1, adl_parse_comma_separated_numerics("1", numbers));
        CPPUNIT_ASSERT_EQUAL(1, numbers[0]);
    }

    void testConvertCSVConvertsTwoCSVNumbers()
    {
        CPPUNIT_ASSERT_EQUAL((uint8_t)2, adl_parse_comma_separated_numerics("1,2", numbers));
        CPPUNIT_ASSERT_EQUAL(1, numbers[0]);
    ...
    {
        CPPUNIT_ASSERT_EQUAL((uint8_t)3, adl_parse_comma_separated_numerics("1,2,3", numbers));
        CPPUNIT_ASSERT_EQUAL(1, numbers[0]);
        CPPUNIT_ASSERT_EQUAL(2, numbers[1]);
