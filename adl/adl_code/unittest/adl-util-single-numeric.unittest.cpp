...
class SingleNumericTest : public CppUnit::TestFixture { 
    ...
    {
        int32_t num;
        CPPUNIT_ASSERT(!adl_parse_single_numeric(NULL, num, &p_result));
    }

    void testConvertSingleNumericReturnsFalseForEmptyString()
    {
        int32_t num;
        CPPUNIT_ASSERT(!adl_parse_single_numeric("", num, &p_result));
    }

    void testConvertSingleNumericReturnsFalseForNonNumericString()
    {
        int32_t num;
        CPPUNIT_ASSERT(!adl_parse_single_numeric("A", num, &p_result));
    }

    void testConvertSingleNumericReturnsTrueForNumericString()
    {
        int32_t num;
        CPPUNIT_ASSERT(adl_parse_single_numeric("0", num, &p_result));
        CPPUNIT_ASSERT_EQUAL(num, 0);
        CPPUNIT_ASSERT(adl_parse_single_numeric("1", num, &p_result));
