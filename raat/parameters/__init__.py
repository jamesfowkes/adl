...
class GroupedParams(namedtuple("GroupedParams", ["base_param", "parameters", "count"])):
    ...
    def get(cls, base_parameter_def):
        base_param = parameters_plugin_manager.getPluginByName(base_parameter_def.type).plugin_object.get(base_parameter_def)
...
def get_single_parameter(parameter_def):
    get_module_logger().info("Trying to load parameter '%s' (%s)", parameter_def.name, parameter_def.type)
...
def get_named_grouped_parameters(parameter_def):
    ...
    for i in range(0, count):
        copied = parameter_def._replace(name="{:s}{:02d}".format(parameter_def.name, i))
