...
class IntegerParam(TemplatedParameter, namedtuple("IntegerParam", ["name", "type", "init_value", "min", "max", "clip", "use_eeprom"])):
...
class IntegerPlugin(IPlugin):
    ...
    def get(self, param):
        ...
        return IntegerParam(param.name, 
            ...
            param.settings.get("min", Setting("min", "", _type.min)),
            param.settings.get("max", Setting("max", "", _type.max)),
