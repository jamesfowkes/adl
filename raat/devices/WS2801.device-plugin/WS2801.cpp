...
void WS2801::set_pixels(uint8_t range_min, uint8_t range_max, RGBParam<uint8_t>& param)
{
    uint8_t rgb[3];
    param.get(rgb);
...
void WS2801::set_pixels(uint8_t range_min, uint8_t range_max, uint8_t r, uint8_t g, uint8_t b)
{
...
void WS2801::set_pixels(uint8_t range_min, uint8_t range_max, const uint8_t rgb[3])
{
    this->set_pixels(range_min, range_max, rgb[0], rgb[1], rgb[2]);
}

int WS2801::handle_command(char const * const command, char * reply)
{
    int32_t rgb[3];
    ...
    if (valid_range && (*end_of_range == ':'))
    {
        ...
        if (parsed_count == 3)
        {
            ...
            {
                this->set_pixels(range_min, range_max, (uint8_t)rgb[0], (uint8_t)rgb[1], (uint8_t)rgb[2]);
                this->show();
...
void WS2801::setPixelColor(uint16_t n, RGBParam<uint8_t>& param)
{
    uint8_t rgb[3];
    param.get(rgb);
...
void WS2801::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b, uint8_t w)
{
...
WS2801::WS2801(uint8_t data_pin, uint8_t clock_pin, uint16_t npixels) : 
    m_pixels(npixels, data_pin, clock_pin), m_npixels(npixels)
{
    m_pixels.begin();
}
...
uint16_t WS2801::command_handler(char const * const command, char * reply)
{
    int reply_length;
...
Adafruit_WS2801& WS2801::pixels()
{
    return m_pixels;
}

uint8_t WS2801::npixels()
{
    return m_npixels;
}
