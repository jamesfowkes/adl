...
class GroupedDevices(namedtuple("GroupedDevices", ["base_device", "devices", "count"])):
    ...
    def get(cls, base_device_def):
        base_device = devices_plugin_manager.getPluginByName(base_device_def.type).plugin_object.get(base_device_def)
...
def get_named_grouped_devices(device_def):
    ...
    for i in range(0, count):
        ...
        devices.append(devices_plugin_manager.getPluginByName(device_def.type).plugin_object.get(copied))
    return devices

def get(device_defs):
    single_devices = [get_single_device(device_def) for device_def in device_defs.single]
