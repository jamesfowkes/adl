...
typedef enum display_width DISPLAY_WIDTH;
...
typedef enum display_lines DISPLAY_LINES;
...
class SparkfunSerialLCD : public SoftwareSerial, public DeviceBase {
public:
    SparkfunSerialLCD (int pin, DISPLAY_WIDTH eWidth, DISPLAY_LINES eLines);
    ...
    void setSplash();
    void setType(DISPLAY_WIDTH eWidth, DISPLAY_LINES eLines, bool write_to_lcd=false);  // new in 1.6
...
private:
    ...
    int handle_config_command(char const * const command, char * reply);
    int handle_text_command(char const * const command, char * reply);
